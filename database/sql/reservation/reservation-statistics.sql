...
/** This depends on: base-log_users.sql **/
start transaction;
...
create table s_tables.t_statistics_http_status_codes (
  ...
  constraint cf_statistics_http_status_codes_code foreign key (code) references s_tables.t_log_http_status_codes (id) on delete restrict on update cascade
);
...
/** create an auto-update trigger. set the role to r_reservation_manager so that the function runs as that role when using "SECURITY DEFINER". The r_reservation_manager must also have the appropriate create privileges. **/
